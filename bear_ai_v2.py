...
class Blokus:
    ...
    def utility(self, state, progress):
        ...
        if state.to_move.board.moves_played <100:
            total_breakdown = [(0.8-progress*0.04)*state.p1.hand_score,state.p1.score,sum([p.size for p in state.p1.pieces]),\
                               state.p1.board.potential_breakdown[1],state.p1.board.potential_breakdown[4],0.75*state.p1.board.resource_value]  # -0.3*state.p1.score_breakdown[0] 
        else:        
            total_breakdown = [state.p1.score]
...
def main():
    ...
    Games = 10
    multi_run(Games, Human_Player, Paddington);
